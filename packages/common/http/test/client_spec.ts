  ...
  describe('HttpClient', () => {
    ...
    describe('makes a basic request', () => {
      ...
      it('params as object', (done: DoneFn) => {
        client.get('/test', {params: {'string': 'string', number: 1, array: [2, 3, "4"], bool: true, a: null}}).subscribe(() => done());
        backend.expectOne('/test?string=string&number=1&array=2&array=3&array=4&bool=true&a=null').flush({});
      });
      it('params as string', (done: DoneFn) => {
        client.get('/test', {params: 'angular=awesome&angular5=yes&a=null'}).subscribe(() => done());
        backend.expectOne('/test?angular=awesome&angular5=yes&a=null').flush({});
      });
      it('params as string with `?` prefix', (done: DoneFn) => {
        client.get('/test', {params: '?angular=awesome&angular5=yes'}).subscribe(() => done()); // With '?' prefix
